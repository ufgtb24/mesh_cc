...
#include <time.h>
Feature_Processor::Feature_Processor(string graph_path, bool use_GPU, string python_path,
	string script_name):Mesh_Processor(graph_path, use_GPU)
	...
	int init_K, PartID part_id, float** output)
{
	...
	int ouput_size;
	float* feat_local = run_graph((float*)(vertice_np->data), adj, pt_num, init_K, ouput_size);
...
float* Feature_Processor::run_graph(float* vertice, int* adj, int pt_num, int init_K, int& out_size) {
